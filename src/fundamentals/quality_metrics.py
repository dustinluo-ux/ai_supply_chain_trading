...
def compute_quality_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    market_cap = out["_market_cap"].where(
        out["_market_cap"].notna() & out["_market_cap"].gt(0)
    )
    out["fcf_yield"] = out["_fcf_ttm"] / market_cap

    current_liab_excess = (out["_current_liabilities"] - out["_short_term_debt"]).clip(
        lower=0
    )
    invested_capital = (out["_total_assets"] - out["_cash"] - current_liab_excess).clip(
        lower=1.0
    )
    ...
    net_income_nonzero = out["_net_income"].where(
        out["_net_income"].notna() & out["_net_income"].ne(0)
    )
    out["fcf_conversion"] = out["_fcf_ttm"] / net_income_nonzero

    valid_revenue = out["_revenue"].where(
        out["_revenue"].notna() & out["_revenue"].gt(0)
    )
    out["net_capex_sales"] = (
        out["_capex"].abs() + out["_r_and_d"].fillna(0)
    ) / valid_revenue

    valid_ebitda = out["_ebitda"].where(out["_ebitda"].notna() & out["_ebitda"].gt(0))
    out["net_debt_ebitda"] = (
        out["_short_term_debt"] + out["_long_term_debt"] - out["_cash"]
    ) / valid_ebitda
