    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            try:
                if DEBUG_MODE:
                    logger.debug(
                        f"[FILE OP] Starting {operation_name}: {func.__name__}"
                    )
                ...
                if DEBUG_MODE:
                    logger.debug(
                        f"[FILE OP] Completed {operation_name}: {func.__name__}"
                    )
                ...
                error_msg = (
                    f"Permission denied in {operation_name} ({func.__name__}): {e}"
                )
                logger.error(error_msg)
...
class ProgressLogger:
    ...
    def start(self, total_items: Optional[int] = None):
        ...
        if total_items:
            logger.info(
                f"[PROGRESS] Starting {self.operation_name} ({total_items} items)"
            )
        else:
    ...
    def update(
        ...
        if current_time - self.last_log_time >= self.interval_seconds:
            ...
            if current_index is not None and total is not None:
                logger.info(
                    f"[PROGRESS] Still processing {self.operation_name}: {current_item} ({current_index}/{total}) - {elapsed:.1f}s elapsed"
                )
            else:
                logger.info(
                    f"[PROGRESS] Still processing {self.operation_name}: {current_item} - {elapsed:.1f}s elapsed"
                )
...
def with_progress_logging(
    interval_seconds: int = 10, operation_name: str = "operation"
):
    """
