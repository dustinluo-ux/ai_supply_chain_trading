...
def analyze_growth_and_reinvestment(metrics: list, line_items: list) -> dict[str, Any]:
    ...
    revs = [m.revenue for m in reversed(metrics) if getattr(m, "revenue", None) is not None and m.revenue]
...
def analyze_risk_profile(metrics: list, line_items: list) -> dict[str, Any]:
    ...
    cost_of_equity = estimate_cost_of_equity(beta if isinstance(beta, (int, float)) else None)
...
def analyze_relative_valuation(metrics: list) -> dict[str, Any]:
    ...
    pes = [m.price_to_earnings_ratio for m in metrics if getattr(m, "price_to_earnings_ratio", None) is not None]
    pes = [p for p in pes if p is not None and p > 0 and not (isinstance(p, float) and np.isnan(p))]
    if len(pes) < 3:
...
def calculate_intrinsic_value_dcf(metrics: list, line_items: list, risk_analysis: dict) -> dict[str, Any]:
    ...
    revs = [m.revenue for m in reversed(metrics) if getattr(m, "revenue", None) and m.revenue]
    if len(revs) >= 2 and revs[0] > 0:
...
def _build_snapshots(
    ...
    if financials is None or financials.empty:
        ni0 = safe_float(info.get("netIncomeToCommon")) if info.get("netIncomeToCommon") is not None else None
        pe0 = safe_float(info.get("trailingPE")) if info.get("trailingPE") is not None else None
        ...
        m0 = SimpleNamespace(
            ...
            free_cash_flow=safe_float(info.get("freeCashflow")) if info.get("freeCashflow") is not None else None,
...
def anchor_ticker(ticker: str, analysis_date: str) -> DamodaranAnchorResult:
    ...
    try:
        ...
        if intrinsic_dec is not None and market_cap is not None and market_cap != 0:
