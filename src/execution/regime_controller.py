...
class RegimeController:
    def __init__(
        self, prices_dict: dict[str, pd.DataFrame], as_of: Any | None = None
    ) -> None:
    ...
    def compute(self, as_of_date: Any) -> dict[str, Any]:
        risk = self._risk_overlay.evaluate(
            as_of_date if as_of_date is not None else self._as_of
        )
    ...
    def write_regime_status(
        self, regime_state_dict: dict[str, Any], path: Path
    ) -> None:
        ...
        payload["score_floor"] = float(
            payload.get("score_floor", 0.65 if is_contraction else 0.50)
        )
        payload["allocation_multiplier"] = float(
            payload.get("multiplier", 0.6 if is_contraction else 1.0)
        )
