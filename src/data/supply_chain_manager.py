...
class SupplyChainManager:
    ...
    def _load_database(self) -> Dict:
        ...
        if not self.db_path.exists():
            ...
            logger.warning(
                "[SupplyChainManager] relationships file not found at %s — supply chain propagation disabled",
                self.db_path,
            )
            return {
                "metadata": {
    ...
    def ensure_coverage(
        self, stocks: List[str], max_age_months: int = 6, auto_research: bool = True
    ) -> Dict[str, str]:
        """
    ...
    def _research_and_add(self, ticker: str) -> bool:
        ...
        with open(queue_file, "a", encoding="utf-8") as f:
            f.write(
                f"{ticker} - Added {datetime.now().isoformat()[:10]} - Auto-research failed\n"
            )
    ...
    def _update_ticker(self, ticker: str) -> bool:
        ...
        try:
            ...
            if filing:
                ...
                _existing_customers = {  # noqa: F841
                    c.get("ticker"): c for c in rel_data.get("customers", [])
                }
    ...
    def get_related_companies(self, ticker: str) -> List[str]:
        ...
        related.extend(
            [
                s["ticker"]
                for s in suppliers
                if s.get("ticker") and s["ticker"] != "UNKNOWN"
            ]
        )
        ...
        related.extend(
            [
                c["ticker"]
                for c in customers
                if c.get("ticker") and c["ticker"] != "UNKNOWN"
            ]
        )
    ...
    def get_coverage_report(self) -> Dict:
        ...
        # Count stale
        stale_count = sum(1 for ticker in relationships.keys() if self.is_stale(ticker))
