...
class SECFilingParser:
            ...
            for entry in companies.values():
                if entry.get("ticker", "").upper() == ticker.upper():
                    ...
                    if isinstance(cik_str, int):
                        ...
                        return cik_str.lstrip("0").zfill(
                            10
                        )  # Remove leading zeros, pad to 10
                    else:
    ...
    def extract_customer_concentration(self, filing_text: str) -> List[Dict]:
        ...
        for pattern in patterns:
            ...
            for match in matches:
                ...
                if not pct_match:
                    pct_match = re.search(
                        r"(\d+).*?percent", match.group(), re.IGNORECASE
                    )
    ...
    def extract_supplier_mentions(self, filing_text: str) -> List[Dict]:
        ...
        for keyword in supply_keywords:
            ...
            for match in matches:
                ...
                if (
                    company_name and len(company_name.split()) <= 4
    ...
    def find_major_customer_candidates(
        self, supplier_ticker: str, potential_customers: List[str]
    ) -> List[Dict]:
        """
        ...
        for concentration in concentrations:
            if concentration["likely_major_customer"]:
                ...
                _context_lower = concentration["context"].lower()  # noqa: F841
