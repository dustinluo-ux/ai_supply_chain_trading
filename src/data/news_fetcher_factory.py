...
class NewsFetcherFactory:
    ...
    def create_source(
        ...
        if source_name_lower not in _SOURCE_MAP:
            ...
            raise ValueError(
                f"Unknown news source: {source_name}. Available: {available}"
            )
    ...
    def create_from_config(cls, config_path: Optional[str] = None) -> NewsDataSource:
        ...
        data_dir = news_config.get(
            "data_dir", "C:/ai_supply_chain_trading/trading_data/news"
        )
...
class NewsFetcher:
    ...
    def fetch_articles_for_ticker(
        ...
        return self.source.fetch_articles_for_ticker(
