...
class PriceFetcher:
    ...
    def __init__(
        ...
        try:
            ...
            logger.info(
                f"PriceFetcher initialized: market cap filter ${min_market_cap:,} - ${max_market_cap:,}"
            )
            ...
            try:
                ...
                if df.empty:
                    if attempt < max_retries - 1:
                        logger.warning(
                            f"No data returned for {ticker}, retrying ({attempt + 1}/{max_retries})..."
                        )
                ...
                if not all(col in df.columns for col in required_cols):
                    if attempt < max_retries - 1:
                        logger.warning(
                            f"Missing required columns for {ticker}, retrying ({attempt + 1}/{max_retries})..."
                        )
                        time.sleep(5)
                        continue
                    logger.warning(
                        f"Missing required columns for {ticker} after {max_retries} attempts"
                    )
                    return None
                ...
                if attempt < max_retries - 1:
                    logger.warning(
                        f"Error fetching {ticker} (attempt {attempt + 1}/{max_retries}): {e}, retrying in 5 seconds..."
                    )
                    time.sleep(5)
                    continue
                logger.error(
                    f"Error fetching data for {ticker} after {max_retries} attempts: {e}"
                )
                return None
    ...
    def get_existing_tickers(self, start_date: str, end_date: str) -> set:
        ...
        try:
            ...
            logger.info(
                f"Found {valid_count} valid existing tickers with complete data"
            )
    ...
    def fetch_all_tickers(
        ...
        if use_cache:
            ...
            logger.info(
                f"Found {len(existing_tickers)} existing tickers with valid data"
            )
        ...
        logger.info(
            f"Downloading price data for {remaining} tickers from {start_date} to {end_date}"
        )

        for idx, ticker in enumerate(
            tqdm(tickers_to_download, desc="Fetching prices"), 1
        ):
            cache_path = os.path.join(self.data_dir, f"{ticker}.parquet")
            ...
            if idx % save_progress_every == 0:
                logger.info(
                    f"Progress: {idx}/{remaining} tickers downloaded ({len(results)} total, {cached_count} cached, {len(failed)} failed)"
                )
                # Save intermediate results
                try:
                    for saved_ticker, saved_df in results.items():
                        if (
                            saved_ticker not in existing_tickers
                        ):  # Only save newly downloaded
                            save_path = os.path.join(
                                self.data_dir, f"{saved_ticker}.parquet"
                            )
        ...
        if failed:
            logger.warning(
                f"Failed tickers ({len(failed)}): {', '.join(failed[:20])}{'...' if len(failed) > 20 else ''}"
            )
    ...
    def run(
        ...
        with open(summary_path, "w") as f:
            ...
            f.write(
                f"Market Cap Filter: ${self.min_market_cap:,} - ${self.max_market_cap:,}\n"
            )
