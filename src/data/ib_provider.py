...
import pandas as pd
import nest_asyncio
from ib_insync import IB, Stock, util, Forex, Crypto
...
class IBDataProvider(BaseDataProvider):
    ...
    def __init__(
        ...
        self._connect()
        logger.info(
            f"IBDataProvider initialized: {host}:{port}, client_id={self.client_id}"
        )
        ...
        **kwargs,
        ...
        if end_date:
            ...
            duration_str = (
                f"{int(years)} Y"
                if years >= 1
                else f"{int((end_dt - start_dt).days)} D"
            )
        else:
            duration_str = "2 Y"
    ...
    def get_realtime_volume(
        self, ticker: str, apply_us_equity_multiplier: bool = True
    ) -> float:
        """
        ...
        try:
            ...
            ticker_obj = self.ib.reqMktData(
                contract, genericTickList="8", snapshot=False
            )
            self.ib.sleep(1)
            vol = 0.0
    ...
    def get_account_info(self) -> Dict:
        ...
        try:
            ...
            return {"margin_info": margin_info, "positions": pos_list}
        except Exception as e:
