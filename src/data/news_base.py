...
class NewsDataSource(ABC):
    ...
    def __init__(
        ...
        logger.info(
            f"{self.__class__.__name__} initialized with {len(self.keywords)} keywords"
        )
    ...
    def _get_cached_articles(
        ...
        try:
            ...
            for article in cached:
                ...
                article_date = (
                    article_date.tz_convert("UTC").tz_localize(None)
    ...
    def _standardize_article(
        self, article: Dict, ticker: str, source_name: str
    ) -> Dict:
        """
    ...
    def fetch_all_tickers(
        ...
        for ticker in tqdm(tickers, desc=f"Fetching news ({self.get_name()})"):
            try:
                articles = self.fetch_articles_for_ticker(
