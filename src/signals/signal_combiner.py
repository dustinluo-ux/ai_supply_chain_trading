...
class SignalCombiner:
    ...
    def load_technical_indicators(
        self, date: Optional[str] = None, include_rsi: bool = False
    ) -> pd.DataFrame:
    ...
    def calculate_technical_only_signal(
        self, date: Optional[str] = None, min_price_momentum: float = 0.0
    ) -> pd.DataFrame:
        """
        ...
        if "rsi" in technical.columns:
            ...
            technical["rsi_score"] = technical["rsi_score"].fillna(
                0.5
            )  # Neutral if missing
        else:
        ...
        technical["price_momentum_norm"] = self.normalize_signal(
            technical["price_momentum"]
        )
        technical["volume_spike_norm"] = self.normalize_signal(
    ...
    def combine_signals(
        ...
        # Normalize each signal component
        combined["supply_chain_norm"] = self.normalize_signal(
            combined["supply_chain_score"]
        )
        combined["sentiment_norm"] = self.normalize_signal(
        ...
        if min_sentiment_momentum is not None:
            combined = combined[
                combined["sentiment_momentum"] >= min_sentiment_momentum
            ]
    ...
    def get_top_stocks(
        ...
        **filter_kwargs,
        ...
        if mode == "technical_only":
            _signal_col = "technical_signal"  # noqa: F841
        else:
            _signal_col = "composite_signal"  # noqa: F841
        ...
        output_path = os.path.join(
            self.output_dir, f"top_stocks_{date or 'latest'}{mode_suffix}.csv"
        )
    ...
    def combine_signals_direct(
        self, technical_signals: Dict, news_signals: Dict, weights: Dict[str, float]
    ) -> float:
        """
        ...
        _rsi = technical_signals.get("rsi_score", 0.5)  # noqa: F841
        ...
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug(
                f"combine_signals_direct: weights={weights}, tech={technical_signals}, news={news_signals}"
            )
        ...
        if total_weight == 0:
            logger.warning(
                "All weights are zero in combine_signals_direct! Returning neutral score."
            )
            return 0.5
