...
FEATURE_REGISTRY: Dict[str, Dict[str, Any]] = {
    ...
    "momentum_avg": {
        "type": "technical",
        "column": None,
        "compute_fn": "_compute_momentum_avg",
        "active": False,
        "protected": False,
    },
    "volume_ratio_norm": {
        "type": "technical",
        "column": None,
        "compute_fn": "_compute_volume_ratio_norm",
        "active": False,
        "protected": False,
    },
    "adx_norm": {
    ...
    "willr_norm": {
        "type": "technical",
        "column": None,
        "compute_fn": "_compute_willr_norm",
        "active": False,
        "protected": False,
    },
    "stoch_k_norm": {
        "type": "technical",
        "column": None,
        "compute_fn": "_compute_stoch_k_norm",
        "active": False,
        "protected": False,
    },
    "stoch_d_norm": {
        "type": "technical",
        "column": None,
        "compute_fn": "_compute_stoch_d_norm",
        "active": False,
        "protected": False,
    },
    "cci_norm": {
    ...
    "momentum_5d_norm": {
        "type": "technical",
        "column": None,
        "compute_fn": "_compute_momentum_5d_norm",
        "active": False,
        "protected": False,
    },
    "momentum_20d_norm": {
        "type": "technical",
        "column": None,
        "compute_fn": "_compute_momentum_20d_norm",
        "active": False,
        "protected": False,
    },
    "bb_position_norm": {
        "type": "technical",
        "column": None,
        "compute_fn": "_compute_bb_position_norm",
        "active": False,
        "protected": False,
    },
    "atr_norm": {
...
def _get_news_features_for_date(
    ticker: str, date: pd.Timestamp, news_signals: Dict
) -> Dict[str, float]:
    ...
    date_str = (
        date.strftime("%Y-%m-%d") if isinstance(date, pd.Timestamp) else str(date)
    )
...
def _compute_stoch_k_norm(df: pd.DataFrame) -> pd.Series:
    ...
    s = (
        st[col[0]].reindex(df.index).fillna(50)
        if col
...
def _compute_rel_momentum_20d(
    df: pd.DataFrame, smh_df: pd.DataFrame, date: pd.Timestamp
) -> Optional[float]:
    """Stock 20-day return minus SMH 20-day return. Positive = stock outperforming sector."""
    try:
        stock_close = (
            df["close"]
            if not isinstance(df["close"], pd.DataFrame)
...
class FeatureSelector:
    ...
    def build_feature_matrix(
        ...
        for ticker in prices_dict:
            ...
            if not all(
                c in full_df.columns for c in ["open", "high", "low", "close", "volume"]
            ):
                continue
            ...
            for date in weekly_dates:
                ...
                for name, meta in FEATURE_REGISTRY.items():
                    if meta["type"] == "technical":
                        col = (
                            ...
                            else (meta.get("column") if meta.get("column") else None)
                        )
                        val = row.get(col, row.get(name, 0.5))
                        feat[name] = float(val) if pd.notna(val) else 0.5
                    elif (
                        meta.get("type") == "relative"
                        and _smh_df is not None
                        and meta.get("smh_compute_fn")
                    ):
                ...
                for k, v in news_feat.items():
                    feat[k] = v
                try:
                    fwd = pipeline._calculate_forward_return(
                        ticker, date, prices_dict, horizon_days=7
                    )
                except Exception:
                    fwd = None
                if fwd is not None and _use_residual and _smh_df is not None:
                    smh_fwd = pipeline._calculate_smh_forward_return(
                        _smh_df, date, horizon_days=7
                    )
                    beta = pipeline._calculate_rolling_beta(
                        prices_dict[ticker], _smh_df, date, window=_beta_window
                    )
                    if smh_fwd is not None:
                        fwd = fwd - (beta * smh_fwd)
                if fwd is None or (
                    isinstance(fwd, float) and (np.isnan(fwd) or np.isinf(fwd))
                ):
                    continue
    ...
    def tournament(
        ...
        # Step 1 — Build matrix
        mat = self.build_feature_matrix(
            prices_dict, news_signals, train_start, train_end
        )
        if mat.empty or len(mat) < 200:
            logger.warning(
                "Feature matrix has %d rows; need >= 200. Returning current active.",
                len(mat),
            )
            return active_names
        ...
        for col in feature_cols:
            ...
            r = (
                float(r)
                if not (r is None or (hasattr(r, "__len__") and len(r) == 0))
                else float("nan")
            )
        ...
        if dropped_low_ic:
            logger.info(
                "IC screen dropped (|IC| < %s): %s", self.ic_threshold, dropped_low_ic
            )
        ...
        protected = [
            f for f in FEATURE_REGISTRY if FEATURE_REGISTRY[f].get("protected")
        ]
        sorted_survivors = sorted(
            survivors, key=lambda x: abs(ic_vals.get(x, 0)), reverse=True
        )
