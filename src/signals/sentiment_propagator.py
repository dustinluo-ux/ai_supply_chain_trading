...
class SentimentPropagator:
    ...
    def calculate_relationship_weight(
        self, relationship: Dict, relationship_type: str
    ) -> float:
        """
    ...
    def propagate(
        ...
        logger.debug(
            "Propagating sentiment from %s (sentiment=%.3f)",
            primary_ticker,
            news_item.sentiment_score,
        )

        while queue:
            current_ticker, current_tier, source_ticker, cumulative_weight = (
                queue.popleft()
            )
            ...
            use_discovered = (
                links_for_primary if (current_ticker == primary_ticker) else None
            )
            ...
            for rel_type in ["suppliers", "customers", "competitors"]:
                ...
                for rel in rel_list:
                    ...
                    if (
                        related_ticker in visited
                        and visited[related_ticker] <= current_tier + 1
                    ):
                        continue
                    ...
                    if current_tier == 0:
                        ...
                        base_weight = self.calculate_relationship_weight(
                            rel, rel_type[:-1]
                        )  # Remove 's' from 'suppliers'
                    ...
                    propagated_supply_chain = (
                        news_item.supply_chain_score * sigmoid_weight
                    )
                    ...
                    signal = PropagatedSignal(
                        ...
                        confidence=rel.get("confidence", "medium") == "high"
                        and 0.7
                        or 0.5,
        ...
        logger.info(
            f"Generated {len(propagated_signals)} propagated signals from {primary_ticker}"
        )
    ...
    def _resolve_entity_to_ticker(
        ...
        try:
            ...
            cfg = get_config()
            entity_map = (
                cfg.get_param("strategy_params.llm_analysis.entity_ticker_map", None)
                or {}
            )
        except Exception:
            entity_map = {}
        ...
        resolved = (
            entity_map.get(normalized)
            or entity_map.get(normalized.title())
            or entity_map.get(entity.strip())
        )
    ...
    def _get_relationships(
        ...
        if discovered_links:
            ...
            logger.info(
                "Merged %d discovered links for %s", len(discovered_links), ticker
            )
    ...
    def propagate_from_news_result(
        self, ticker: str, news_result: Dict
