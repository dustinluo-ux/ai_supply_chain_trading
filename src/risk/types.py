...
@dataclass(frozen=True)
class OptionOrder:
    """A single options hedge instruction."""

    symbol: str
    right: str  # always "P" (put) for tail hedge
    notional_usd: Decimal
    contracts: int  # 0 at plan time; sized at submission time
    reason: str
...
class FinalExecutionPlan:
    ...
    option_orders: list[OptionOrder] = field(default_factory=list)
    audit_log: list[str] = field(default_factory=list)
