...
st.set_page_config(
    page_title="AI SC Trading — Portfolio Monitor", layout="wide", page_icon="📈"
)
...
if not can_rebalance:
    st.warning(
        "Rebalancing BLOCKED — data quality gate failed. Check execution_status.json for details."
    )
...
if alert_rows:
    ...
    for r in alert_rows:
        ...
        cols[3].markdown(
            f'<span style="{_severity_style(r["Severity"])}">{r["Severity"]}</span>',
            unsafe_allow_html=True,
        )
        cols[4].write(r["Status"])
else:
    st.caption(
        "No alert data (structural_breakdown.json and/or drawdown_tracker.json missing)."
    )
...
if aggregator_audit is None:
    ...
    entries = [
        e
        for e in (aggregator_audit.get("conflict_details") or [])
    ...
    if not entries:
        ...
        with st.expander(f"Conflict Details ({len(entries)})"):
            ...
            for e in entries:
                ...
                core_str = (
                    f"{pod_contribs.get('core', 0):.4f}"
                    if "core" in pod_contribs
                    else "—"
                )
                ext_str = (
                    f"{pod_contribs.get('extension', 0):.4f}"
                    if "extension" in pod_contribs
                    else "—"
                )
                ballast_str = (
                    f"{pod_contribs.get('ballast', 0):.4f}"
                    if "ballast" in pod_contribs
                    else "—"
                )
                action_style = (
                    "color: #c00000;" if action == "vetoed" else "color: #b8860b;"
                )
                cols = st.columns([1.5, 1.2, 1.2, 1, 1, 1])
                cols[0].write(ticker)
                cols[1].markdown(
                    f'<span style="{action_style}">{action}</span>',
                    unsafe_allow_html=True,
                )
...
frozen_pods = (
    frozen_data.get("frozen_pods") if isinstance(frozen_data, dict) else None
) or {}
if not frozen_data:
    st.markdown(
    ...
    st.caption(
        "To unfreeze: add pod name to config/strategy_params.yaml → pod_overrides → frozen_pods_resume"
    )
...
if regime_data is None:
    ...
    with c2:
        st.metric(
            "SPY Close",
            (
                f"{regime_data.get('spy_close', 0):.2f}"
                if regime_data.get("spy_close") is not None
                else "—"
            ),
        )
...
with col_right:
    ...
    if ps is None:
        ...
        non_zero = [
            (t, h.get("shares", 0), h.get("avg_cost", 0))
            for t, h in holdings.items()
            if (h.get("shares") or 0) > 0
        ]
        rows = [
            {"Ticker": t, "Shares": s, "Avg Cost": f"{c:.2f}" if c else "—"}
            for t, s, c in non_zero
        ]
        if rows:
            st.dataframe(rows, width="stretch", hide_index=True)
        else:
...
if ls is None:
    ...
    if rows:
        rows.sort(
            key=lambda r: (r["Score"] if r["Score"] != "—" else "0"), reverse=True
        )
        st.dataframe(rows, width="stretch", hide_index=True)
    else:
...
with ml_col2:
    ...
    if ic_data is None or not isinstance(ic_data, list):
        ...
        entries = (
            ...
            else (ic_data.get("entries") or ic_data)
        )
...
if not fills:
    ...
    miss_count = sum(
        1 for r in fills if (r.get("qty_filled") or 0) < (r.get("qty_requested") or 0)
    )
    if miss_count > 0:
        ...
        rows.append(
            {
                ...
                "Qty Filled": qty_f,
                "Avg Fill Price": (
                    f"{r.get('avg_fill_price'):.2f}"
                    if r.get("avg_fill_price") is not None
                    else "—"
                ),
                "Status": r.get("status", "—"),
