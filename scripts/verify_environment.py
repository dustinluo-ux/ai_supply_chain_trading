...
def _check_gemini_api_key() -> tuple[bool, str]:
    ...
    if (
        ...
        and key.strip()
        and not key.strip().lower().startswith(("your_", "xxx"))
    ):
        return True, "set (Gemini API ready)"
    return (
        False,
        "not set or placeholder (set GOOGLE_API_KEY or GEMINI_API_KEY in .env for LLM bridge)",
    )
...
def main() -> int:
    ...
    if not hasattr(np, "NaN"):
        np.NaN = (
            np.nan
        )  # numpy 2.x; pandas_ta imports numpy.NaN before technical_library patch runs
    ...
    print(
        f"  Gemini API key (GOOGLE_API_KEY / GEMINI_API_KEY): {gemini_msg}", flush=True
    )

    if missing:
        print(f"\nERROR: Missing required packages: {', '.join(missing)}", flush=True)
        print(
            "  conda env update -f environment.yml  # or: conda activate wealth",
            flush=True,
        )
    ...
    if not gemini_ok:
        print(
            "\nEnvironment OK for parity/regression. Gemini bridge will use safety fallback until API key is set.",
            flush=True,
        )
    else:
        print(
            "\nEnvironment OK for parity, regression, and Gemini (LLM Active).",
            flush=True,
        )
    return 0
