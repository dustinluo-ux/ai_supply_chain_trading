...
def _check_1_and_2() -> tuple[bool, str]:
    ...
    try:
        ...
        if r0.get("qty_requested") != 100 or r0.get("qty_filled") != 100 or r0.get("status") != "full":
            return False, f"Record A fields wrong: {r0}"
        if r1.get("qty_requested") != 50 or r1.get("qty_filled") != 30 or r1.get("status") != "partial":
...
def _check_3() -> tuple[bool, str]:
    ...
    try:
        from src.execution.fill_ledger import read_fill_ledger
        records = read_fill_ledger(path=temp_path)
        misses = [r for r in records if r.get("qty_filled", 0) < r.get("qty_requested", 0)]
        if len(misses) != 1:
...
def main() -> int:
    ...
    if not result_1_and_2[0]:
        ...
        print("PASS 1: temp fills.jsonl written with 2 records (A full, B partial)", flush=True)
        print("PASS 2: read_fill_ledger returned both records with correct fields", flush=True)
    ok3, msg3 = _check_3()
