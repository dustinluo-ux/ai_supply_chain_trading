...
def _build_rank_label(meta_df):
    ...
    meta_df["pct"] = meta_df.groupby("date")["forward_return"].rank(
...
def main():
    ...
    with open(cfg_path, "r") as f:
        _cfg = yaml.safe_load(f)  # noqa: F841
    ...
    def get_xy(start, end):
        ...
        for t in prices_dict:
            for d in pd.date_range(start, end, freq="W-MON"):
                f = pipeline.extract_features_for_date(
                    t, pd.Timestamp(d), prices_dict, news_signals
                )
                if f is None:
                    continue
                ...
                if r is None:
                    continue
                X_list.append(f)
                meta.append({"date": d, "forward_return": r})
        if not X_list:
            return None, None, None
        X = np.array(X_list)
        df = pd.DataFrame(meta)
        y = _build_rank_label(df)
        return X, y, df

    X_train, y_train, _ = get_xy("2022-01-01", "2024-12-31")
    if X_train is None or len(X_train) < 50:
        print("ERROR: Insufficient train data.")
        return 1
    ...
    for name, model in [("XGBoost", xgb), ("CatBoost", cat)]:
        ...
        for s, e in folds:
            ...
            if X_t is None or len(X_t) < 5:
                ics.append(0.0)
                continue
            pred = model.predict(X_t)
            ic, _ = spearmanr(pred, y_t)
            ics.append(float(ic) if not np.isnan(ic) else 0.0)
        mean_ic = np.mean(ics)
        n_pos = sum(1 for i in ics if i > 0)
    ...
    print("\n--- DUEL RESULTS ---")
    print(
        f"{'Model':<12} {'Q1':>8} {'Q2':>8} {'Q3':>8} {'Q4':>8} {'Mean IC':>8} {'Pass':>6}"
    )
    for name in ["XGBoost", "CatBoost"]:
        r = results[name]
        f = r["folds"]
        print(
            f"{name:<12} {f[0]:>8.4f} {f[1]:>8.4f} {f[2]:>8.4f} {f[3]:>8.4f} {r['mean_ic']:>8.4f} {'YES' if r['passed'] else 'NO':>6}"
        )
    winner = None
    if results["XGBoost"]["passed"] and results["CatBoost"]["passed"]:
        winner = (
            "XGBoost"
            if results["XGBoost"]["mean_ic"] >= results["CatBoost"]["mean_ic"]
            else "CatBoost"
        )
    elif results["XGBoost"]["passed"]:
        winner = "XGBoost"
    elif results["CatBoost"]["passed"]:
        winner = "CatBoost"
    if winner:
        import joblib
        ...
        if "tracks" not in c:
            c["tracks"] = {}
        if "A" not in c["tracks"]:
            c["tracks"]["A"] = {}
        c["tracks"]["A"]["model_path"] = str(save_path.resolve())
        c["use_ml"] = True
        with open(cfg_path, "w") as f:
            yaml.dump(
                c, f, default_flow_style=False, sort_keys=False, allow_unicode=True
            )
        print(
